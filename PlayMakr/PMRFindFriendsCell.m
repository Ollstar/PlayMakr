...
#import "PMRFindFriendsCell.h"
#import "PMRProfileImageView.h"
#import "PMConstants.h"


@interface PMRFindFriendsCell ()
...
@implementation PMRFindFriendsCell
@synthesize delegate;
@synthesize user;
...
- (void)setUser:(PFUser *)aUser {
    ...
    // Configure the cell
    [avatarImageView setFile:[self.user objectForKey:kPMUserProfilePicSmallKey]];
    
    // Set name 
    NSString *nameString = [self.user objectForKey:kPMUserDisplayNameKey];
    
    CGRect nameRect = [nameString boundingRectWithSize:CGSizeMake(144.0f, CGFLOAT_MAX) options:NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:16.0f]} context:nil];
    CGSize nameSize = nameRect.size;
    CGFloat nameWidth = ceilf(nameSize.width);
    CGFloat nameHeight = ceilf(nameSize.height);

    [nameButton setTitle:[self.user objectForKey:kPMUserDisplayNameKey] forState:UIControlStateNormal];
    [nameButton setTitle:[self.user objectForKey:kPMUserDisplayNameKey] forState:UIControlStateHighlighted];

    [nameButton setFrame:CGRectMake( 60.0f, 17.0f, nameWidth, nameHeight)];
    
    // Set photo number label
    
    CGRect photoLabelRect = [@"photos" boundingRectWithSize:CGSizeMake(144.0f, CGFLOAT_MAX) options:NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:11.0f]} context:nil];
    CGSize photoLabelSize = photoLabelRect.size;
    CGFloat photoLabelWidth = ceilf(photoLabelSize.width);
    CGFloat photoLabelHeight = ceilf(photoLabelSize.height);
    

    [photoLabel setFrame:CGRectMake( 60.0f, 17.0f + photoLabelWidth, 140.0f, photoLabelHeight)];
